...
void generate_arrays(double* M1, double* M2, int n, unsigned* seed) {
 	...
 	int i = 0;
 	#pragma omp parallel for num_threads(2) shared(i, M1) schedule(guided, 1)
	for (i = 0; i < n; i++) {
...
void map(double* M1, double* M2, int n) {
	...
	#pragma omp parallel for num_threads(2) private(tmp) shared(i) schedule(guided, 1)
	for (i = 0; i < n; i++) {
		tmp = cbrt(M1[i] / M_E);
		M1[i] = tmp;
	}
 
	#pragma omp parallel for num_threads(2) shared(i) schedule(guided, 1)
...
void merge(double* M1, double* M2, int n) {
	...
	int i;
	#pragma omp parallel for num_threads(2) private(i) shared(M1, M2) schedule(guided, 1)
	for (i = 0; i < n/2; i++) {
		M2[i] = abs(M1[i] - M2[i]);
	}
}
...
void comb_sort(double* new_array, int n){
	...
	while (gap != 1 || swapped == true) {
		...
		int i;
		#pragma omp parallel for num_threads(2) private(temp) shared(i, gap) schedule(guided, 1)
		for (i = 0; i < n-gap; i++) {
 ...
 double min_element(double* array, int n) { //works only for positive numbers
 	...
 	#pragma omp parallel for num_threads(2) reduction(min: minimum) schedule(guided, 1)
 	// how to return the minimum from N threads?
	for (int i = 1; i < n; i++)
		if (array[i] < minimum)
			minimum = array[i];
...
void reduce(double * M2, int n) {
	...
	#pragma omp parallel for num_threads(2) private(mod_element, mod_element_int) reduction(+: sum) schedule(guided, 1)
	for (int i = 0; i < n; i++) {
		mod_element = M2[i] / min;
		mod_element_int = (int)floor(mod_element);
